import React from 'react'

// Registry of level components. Add new levels here using lazy imports.
// Example: '2': React.lazy(() => import('../Level_2'))
export const levels: Record<string, React.LazyExoticComponent<React.ComponentType<any>>> = {
  '1': React.lazy(() => import('../Level_1')),
  '1_correct': React.lazy(() => import('../Level_1_correct')),
  '2': React.lazy(() => import('../Level_2')),
  '2_correct': React.lazy(() => import('../Level_2_correct')),
  '3': React.lazy(() => import('../Level_3')),
  '3_correct': React.lazy(() => import('../Level_3_correct')),
  '4': React.lazy(() => import('../Level_4')),
  '4_correct': React.lazy(() => import('../Level_4_correct'))
  ,
  '5': React.lazy(() => import('../Level_5')),
  '5_correct': React.lazy(() => import('../Level_5_correct'))
  ,
  '6': React.lazy(() => import('../Level_6')),
  '6_correct': React.lazy(() => import('../Level_6_correct'))
  ,
  '7': React.lazy(() => import('../Level_7')),
  '7_correct': React.lazy(() => import('../Level_7_correct'))
  ,
  '8': React.lazy(() => import('../Level_8')),
  '8_correct': React.lazy(() => import('../Level_8_correct'))
}

export function getLevelComponent(id?: string) {
  if (!id) return undefined
  return levels[id]
}

export default levels