...
const LevelTitle: React.FC<LevelTitleProps> = ({ eyebrow, title, description, onClose, large }) => {
  ...
  // previously used close button styles removed — styles are handled via CSS classes

  return (
    <div className={`level-title uh-level-tittle ${large ? 'level-title--large' : ''}`} style={containerStyle}>
      {onClose && (
        <button onClick={onClose} aria-label="Ir al nivel" className="level-title__go-button">
