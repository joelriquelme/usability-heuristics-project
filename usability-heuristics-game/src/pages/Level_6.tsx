...
const Level6: React.FC = () => {
  ...
  return (
    <div className="uh-card level-6">
      <div className="level-6__header">
        <h2>Creación de Cuenta</h2>
        <p className="level-6__subtitle">Asistente de configuración</p>
      </div>

      <div className="level-6__steps" data-eval="show" question-id="level-6-step-indicators">
        {[1, 2, 3].map((step) => (
          <React.Fragment key={step}>
            {step > 1 && <span className="level-6__step-divider">—</span>}
            <span
              className={`level-6__step-badge ${currentStep === step && !accountCreated ? 'active' : ''} ${accountCreated || currentStep > step ? 'completed' : ''}`}
            >
              {step}
            </span>
          </React.Fragment>
      ...
      <div className="level-6__footer">
        <div className="level-6__actions" data-eval="show" question-id="level-6-back-button">
          {accountCreated ? (
